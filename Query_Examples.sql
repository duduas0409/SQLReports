...
/* show the number of orders shipped to North America and 
the number of orders shipped to other places in separate rows,	 */
...
/* Count the orders processed by employees from the 'WA'
region and by all other employees. Show two columns: employee_region (either 'WA' or 'Not WA'), and order_count. */
...
/* Count the number of products with low, average, and high availability.
Show two columns: availability ('Low' for 10 units or less, 'Average' for 
10 to 30 units, or 'High' for more than 30 units) and product_count.	     */
...
/* Calculate the total revenue generated (after discount) by customers from France and by customers from other countries. 
Show two columns: customer_country (either 'France' or 'Other') and discount_revenue. Round the second column to two decimal places.	*/
...
/*M2 Count the number of vegetarian and non-vegetarian (id 6 & 8) products. Show two columns: non_vegetarian_count and vegetarian_count.	 */
...
/* Count the total number of line items from orders shipped to 
Canada that were 1) sold at full price, or 2) sold at a discounted price.
...
/* For each employee, find the total revenue before discount generated by the orders processed 
for two order groups: dach_orders (orders shipped to Germany, Austria, or Switzerland) and other_orders
(orders shipped to all other countries).
...
/* For each ship_country, show the percentage of revenue for all orders before discount that has been generated by the employees 
with the IDs 1 and 2. Show three columns:
...
/* For each employee, show the percentage of total revenue (before discount) generated by 
orders shipped to the USA and to Germany, with respect to the total revenue generated by that employee. Show the following columns:
employee_id.
first_name.
last_name.
rev_percentage_usa.
...
JOIN germany_revenue ger_rev
  ON us_rev.employee_id = ger_rev.employee_id
JOIN employees e
  ON e.employee_id = us_rev.employee_id;	    
	    
	    

/* Count the number of products with low, average, and high prices. Show two columns:

price_category – low for products with a unit price less than or equal to 30, average for products between 
30 and 90, and high for products with a unit price above 90.
...
/* count how many customers we have in each language group, which is based on the language
used in their country of residence. Show three columns:
...
other_count – the number of customers from other countries.	  */
 ...
 SELECT
  COUNT(CASE
    WHEN country IN ('UK', 'Canada', 'USA', 'Ireland')
      THEN 'English'
  END) AS english_count,
  COUNT(CASE
    WHEN country IN ('Germany', 'Switzerland', 'Austria')
      THEN 'German'
  END) AS german_count,
  COUNT(CASE
    WHEN country NOT IN ('UK', 'Canada', 'USA', 'Ireland', 'Germany', 'Switzerland', 'Austria')
      THEN 'Other'
  END) AS other_count 
FROM customers;


/* Create a report to show the percentage of total revenue after discount generated by orders with low (less than or equal to 90.0) and high
 (greater than 90.0) freight values in each country we ship to.
Show the following columns: ship_country, percentage_low_freight, and percentage_high_freight. */
	    


WITH revenue_low_freight AS (
  SELECT 
    ship_country,
    SUM(CASE
      WHEN freight <= 90.0
...
SELECT 
  lf.ship_country,
  ROUND(lf.freight_revenue / lf.total_revenue * 100, 2) AS percentage_low_freight,
  ROUND(hf.freight_revenue / hf.total_revenue * 100, 2) AS percentage_high_freight
FROM revenue_low_freight lf
JOIN revenue_high_freight hf
  ON lf.ship_country = hf.ship_country;
